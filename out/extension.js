...
function activate(context) {
    let htmlGenerator = function (mappings, staticUris) {
        let commandRowsHTML = "";
        mappings?.forEach(element => {
            let langTagHTML = "";
            commandRowsHTML += "<tr>";
            element.language_tags.split(" ").forEach((tag) => {
    ...
    let commandIdGenerator = function (name) {
        let randomId = name + "_" + Math.random().toString().substring(2, 7);
        let mappingsArray = context.globalState.get("Runner.mappings");
        let mappingsMap = new Map(mappingsArray);
        if (mappingsMap.has(randomId))
            randomId = commandIdGenerator(name);
        return randomId;
    };
    let addMapping = (id, m) => {
        let mappingsArray = context.globalState.get("Runner.mappings");
    ...
    let removeMapping = (id) => {
        let mappingsArray = context.globalState.get("Runner.mappings");
        let mappingsMap = new Map(mappingsArray);
        mappingsMap.delete(id);
        context.globalState.update("Runner.mappings", Array.from(mappingsMap));
    };
    let commandsMappingPanel;
    let commandDispo1 = vscode.commands.registerCommand("Runner.showCommandsMapping", () => {
        if (!commandsMappingPanel) {
            commandsMappingPanel = vscode.window.createWebviewPanel("Runner.CommandsMappingPanel", "Commands Mappings", vscode.ViewColumn.Active, { enableScripts: true, localResourceRoots: [vscode.Uri.file(path.join(context.extensionPath, 'static'))] });
            let loadPanel = function () {
                if (commandsMappingPanel) {
                    let mappingsArray = context.globalState.get("Runner.mappings");
                    let mappings = mappingsArray?.map(element => element[1]);
                    let cssUri = commandsMappingPanel.webview.asWebviewUri(vscode.Uri.file(path.join(context.extensionPath, 'static/css', 'bootstrap.min.css')));
    ...
    let commandDispo2 = vscode.commands.registerCommand("Runner.runCommand", () => {
        let mappingsArray = context.globalState.get("Runner.mappings");
        let docLangId = vscode.window.activeTextEditor?.document.languageId;
        let commandsToShow = [];
        let recentCommandId = context.globalState.get("Runner.recentCommand");
        if (docLangId) {
            let mappingsMap = new Map(mappingsArray);
            mappingsMap.forEach((element) => {
                if (element.language_tags.split(" ").find((tag) => tag == docLangId))
                    commandsToShow?.push(element);
            });
            if (recentCommandId) {
                let recentCommand = mappingsMap.get(recentCommandId);
                if (recentCommand.language_tags.split(" ").find((tag) => tag == docLangId)) {
                    ...
                    commandsToShow?.push(recentCommand);
            }
            mappingsMap.forEach((element) => {
                if (!commandsToShow?.map(commandMap => commandMap.id).find(commandMapId => commandMapId == element.id))
                    commandsToShow?.push(element);
            });
        }
        else {
